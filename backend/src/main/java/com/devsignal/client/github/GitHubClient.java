...
import com.devsignal.dto.github.GitHubReadmeApiDto;
...
public class GitHubClient {
	...
	public GitHubClient(@Value("${github.token:}") String token) {
		...
		if (!trimmedToken.isEmpty()) {
			builder = builder.defaultHeader(HttpHeaders.AUTHORIZATION, "Bearer " + trimmedToken);
		}
	...
	/**
	 * Fetches the default-branch README body as decoded UTF-8 text.
	 * Returns empty when missing (404), inaccessible, or decoding fails — never throws for README-specific failures.
	 */
	public Optional<String> fetchRepositoryReadmeContent(String owner, String repoName) {
		String o = owner == null ? "" : owner.strip();
		String rn = repoName == null ? "" : repoName.strip();
		if (o.isEmpty() || rn.isEmpty()) {
			return Optional.empty();
		}
		try {
			GitHubReadmeApiDto body = restClient.get()
					.uri("/repos/{owner}/{repo}/readme", o, rn)
					.retrieve()
					.body(GitHubReadmeApiDto.class);
			if (body == null || body.content() == null || body.content().isBlank()) {
				return Optional.empty();
			}
			String decoded = decodeReadmeContent(body.content(), body.encoding());
			if (decoded == null || decoded.isBlank()) {
				return Optional.empty();
			}
			return Optional.of(decoded.strip());
		}
		catch (RestClientResponseException ex) {
			return Optional.empty();
		}
	...
	private static String decodeReadmeContent(String content, String encoding) {
		String compact = content.replace("\n", "").replace("\r", "");
		String enc = encoding == null ? "base64" : encoding.strip();
